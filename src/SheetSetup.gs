// Sheet structure installer. Idempotent: re-running adds nothing.
// SHEET_SCHEMA is the source of truth for tab/column layout, dropdowns, and
// default rows for Templates / Config / Dashboard. Keep in sync with
// templates/sheet-schema.json.
  ...
  tabs: {
    Brands: {
      ...
      columns: [
        ...
        {key: 'pitch_angle', header: 'Pitch Angle', type: 'text', width: 240},
        {key: 'status', header: 'Status', type: 'dropdown', values: ['queued','drafted','sent','replied','meeting_booked','won','dead','draft_failed','send_failed','invalid_email'], width: 130},
        {key: 'draft_subject', header: 'Draft Subject', type: 'text', width: 240},
        {key: 'draft_body', header: 'Draft Body', type: 'text', width: 400},
        {key: 'last_action_date', header: 'Last Action Date', type: 'date', width: 110},
    ...
    Dashboard: {
      ...
      columns: [
        ...
        {key: 'value', header: 'Value', type: 'text', width: 160}
      ],
      default_rows: [
        ['Total sent (all time)', '=COUNTIF(Brands!N:N,">=1")'],
        ['Sent this week', '=COUNTIFS(Brands!K:K,">="&(TODAY()-7),Brands!H:H,"sent")'],
        ['Open (sent, no reply yet)', '=COUNTIF(Brands!H:H,"sent")'],
...
function installSheetStructure(ss) {
  ...
  Object.keys(SHEET_SCHEMA.tabs).forEach(function (tabName) {
    var tabSchema = SHEET_SCHEMA.tabs[tabName];
    ...
    if (isFresh) {
      var headers = tabSchema.columns.map(function (c) { return c.header; });
      sheet.appendRow(headers);
      if (tabSchema.default_rows) {
        tabSchema.default_rows.forEach(function (r) { sheet.appendRow(r); });
      }
      if (tabSchema.frozen_rows && sheet.setFrozenRows) sheet.setFrozenRows(tabSchema.frozen_rows);
      tabSchema.columns.forEach(function (c, i) {
        if (c.width && sheet.setColumnWidth) sheet.setColumnWidth(i + 1, c.width);
      });
    }
...
function applyValidations(sheet, columns) {
  ...
  for (var i = 0; i < columns.length; i++) {
    ...
    if (col.type === 'dropdown' && col.values && sheet.getMaxRows) {
      ...
      var rule = SpreadsheetApp.newDataValidation().requireValueInList(col.values, true).build();
